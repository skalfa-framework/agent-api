...
Every major stage transition MUST be recorded in `./.agent.tools/records/ledger.jsonl` as an append-only JSON line. After writing to the ledger, `./.agent.tools/records/state.json` must be compiled/updated to reflect the current active tasks, bugs, and API contracts.

All detailed logs, plans, and code diffs must be stored as separate files in `./.agent.tools/records/activities/` and referenced via relative paths in the ledger payload.
...
*   **Feature Spec Alignment**: If modifying or extending an existing feature, the agent **MUST** check for and read `./.agent.tools/records/features/<feature-slug>.md` to understand the previous implementation details, business logic, and design rationale.
...
*   Create a plan detail file: `./.agent.tools/records/activities/act-<num>-plan-<feature>.md`.
*   Record the event in `./.agent.tools/records/ledger.jsonl`:
    ```json
    {"timestamp": "TIMESTAMP", "agent": "AGENT_NAME", "event": "FEATURE_PLANNED", "payload": {"feature": "FEATURE_NAME", "plan_file": "./.agent.tools/records/activities/act-<num>-plan-<feature>.md"}}
    ```
...
*   Create a review report and diff patch:
    *   Report: `./.agent.tools/records/activities/act-<num>-review-<feature>.md`
    *   Patch: `./.agent.tools/records/activities/act-<num>-diff-<feature>.patch`

### Step 5.2 — Ledger Finalization
*   **Feature Spec Update**: Create (for new features) or update (for modified features) the feature specification file at `./.agent.tools/records/features/<feature-slug>.md` using the standard template:
...
*   Record the completion event in `ledger.jsonl`:
    ```json
    {"timestamp": "TIMESTAMP", "agent": "AGENT_NAME", "event": "FEATURE_COMPLETED", "payload": {"feature": "FEATURE_NAME", "review_file": "./.agent.tools/records/activities/act-<num>-review-<feature>.md", "patch_file": "./.agent.tools/records/activities/act-<num>-diff-<feature>.patch"}}
    ```
*   Update `./.agent.tools/records/state.json` to mark the feature/task as `DONE`.