...
*   If no relevant test file exists, create a new one:
    *   File Path: `./.agent/test/<slug>.test.ts`
...
*   Record the `BUG_FOUND` event in `./.agent/records/ledger.jsonl`:
    ```json
    {"timestamp": "TIMESTAMP", "agent": "AGENT_NAME", "event": "BUG_FOUND", "payload": {"bug_id": "BUG-<num>", "feature": "FEATURE_NAME", "analysis_file": "./.agent/records/activities/act-<num>-bug-analysis-<slug>.md", "test_file": "./.agent/test/<slug>.test.ts"}}
    ```
