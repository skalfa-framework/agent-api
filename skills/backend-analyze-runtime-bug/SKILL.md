...
## Goal
Determine the root cause of backend runtime errors by executing feature-scoped API tests in `.agent.tools/test/` and inspecting runtime logs.

## Allowed Tools & Actions
*   Create and edit test files inside `.agent.tools/test/`.
*   Run tests using Bun:
    ```bash
...
3.  **Locate/Create Test File**:
    *   Check `.agent.tools/test/` for an existing `<slug>.test.ts` file.
...
4.  **Execute Test**:
    *   Run `bun .agent.tools/test/<slug>.test.ts` to reproduce the error.
...
6.  **Report & Record**:
    *   Create a bug analysis report: `.agent.tools/records/activities/act-xxx-bug-analysis-xxx.md`.
    *   Append the `BUG_FOUND` event to `.agent.tools/records/ledger.jsonl`.
    *   Update `.agent.tools/records/state.json` to list the bug as active.
...
*   Skipping test creation or running tests outside `.agent.tools/test/`.
